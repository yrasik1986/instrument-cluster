...
bool CanParser::parseDriveData( const QByteArray &payload, VehicleData &data)
{
    ...
    const std::uint16_t rawSpeed = readUInt16LE(payload, 0);
    const std::uint16_t rpm = readUInt16LE(payload, 2);
    const std::uint8_t ignition = static_cast<std::uint8_t>(payload.at(4));
...
bool CanParser::parseStatusData(const QByteArray &payload, VehicleData &data)
{
    ...
    data.coolantTemperature = static_cast<std::uint8_t>(payload.at(0));

    data.fuel = static_cast<std::uint8_t>(payload.at(1));

    const std::uint8_t status = static_cast<std::uint8_t>(payload.at(2));

    data.leftTurn = (status & (1u << 0)) != 0;

    data.rightTurn = (status & (1u << 1)) != 0;

    data.highBeam = (status & (1u << 2)) != 0;

    data.checkEngine = (status & (1u << 3)) != 0;

    return true;
}

std::uint16_t CanParser::readUInt16LE(const QByteArray &data, int offset)
{
