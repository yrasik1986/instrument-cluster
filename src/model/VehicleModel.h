 ...
 *
 * 1. **Экспоненциальное сглаживание с учётом времени (dt)**
 *    - Формула: `V(t) = V(t-1) + (Target - V(t-1)) * α`
 *    - α вычисляется динамически: `α = 1 - exp(-dt / τ)`
 *    - τ (тау) — постоянная времени (0.35с для скорости, 0.18с для RPM)
 *    - Преимущество: поведение не зависит от частоты таймера
 *
 ...
 * |----------|----------|---------|
 * | SPEED_TAU | 0.35с | Плавность спидометра |
 * | RPM_TAU | 0.18с | Отзывчивость тахометра |
...
private:
    ...
    // Для расчёта реального времени между кадрами
    QElapsedTimer m_dtTimer;
    qint64 m_lastUpdateTime = 0;

    // Постоянные времени (секунды)
    static constexpr double SPEED_TAU = 0.35;
    static constexpr double RPM_TAU = 0.18;
};